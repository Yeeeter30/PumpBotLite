...
async def getcosmetic(ctx: discord.ApplicationContext, search: str):
    ...
    try:
        ...
        if item_data:
            ...
            image_url = item_details.get('images', {}).get('icon', 'No image')
            ...
            await ctx.respond(f"No item found matching: {search}")
    ...
    for item in items_to_display:
        embed.add_field(
            ...
            value=f"**Description:** {item.get('displayDescription', 'No description')}\n"
                  ...
                  f"[Image]({item.get('displayAssets', [{}])[0].get('url', 'N/A')})",
            inline=False
        )
...
async def getitem(ctx: discord.ApplicationContext, search: str):
    ...
    if response.status_code == 200:
        ...
        if weapon:
            ...
            embed.set_image(url=weapon['images']['background'])
            
            await ctx.respond(embed=embed)
        else:
            await ctx.respond(f"No weapon found with ID or Name: {search}")
    else:
...
async def upcoming(ctx: discord.ApplicationContext, page: int = 1):
    ...
    if response.status_code == 200:
        ...
        for item in page_items:
            ...
            item_background = item['images']['full_background']
